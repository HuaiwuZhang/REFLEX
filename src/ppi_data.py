...
try:
    ...
    esm = None
    print("Warning: esm package not found. ESM-2 tokenizer will be unavailable.")

def init_esm2_tokenizer(model_name: str = "esm2_t33_650M_UR50D"):
    """
      - esm2_t12_35M_UR50D
      - esm2_t30_150M_UR50D
      - esm2_t33_650M_UR50D etc.
    """
    if esm is None:
        raise ImportError("esm package is not installed. Please 'pip install fair-esm' or 'pip install esm'.")
...
class PPIData(object):
	def __init__(self,args,class_num = 7):
		self.args,self.seqPath, self.relPath,self.mapPath = args,args.sequence,args.action,args.structure
		if hasattr(args, 'split_seed') and args.split_seed is not None:
			print(f"[Data] Setting split seed to {args.split_seed}")
		...
		prot_node, prot_edge, prot_kneg = read_structure_data(args.structure,args.input_path)

		self.prot_encode, self.prot_token_ids, self.vocab_size, self.pad_value, self.alphabet = self._get_protein_encoding(args)
	...
	def encode_with_esm2(
		...
		if cache_path is not None and os.path.isfile(cache_path):
			print(f"[ESM2] load cached embeddings from {cache_path}")
			prot_encode = torch.load(cache_path, map_location="cpu")
			if prot_encode.shape[0] != len(self.seqs):
				print(
					f"[ESM2] cached embedding num {prot_encode.shape[0]} "
					f"!= seq num {len(self.seqs)}, ignore cache and recompute."
				)
			else:
				return prot_encode
		...
		if num_layers is None:
			raise ValueError("esm_model has no num_layers attribute, check ESM version.")

		if layer == -1:
			use_layer = num_layers
		else:
			if not (0 < layer <= num_layers):
				raise ValueError(f"repr layer {layer} out of range (1..{num_layers})")
			use_layer = layer

		name_seq_pairs = [(f"prot_{idx}", seq) for idx, seq in enumerate(self.seqs)]

		batch_size = 1
		all_repr = []

		print(f"[ESM2] computing embeddings with layer={use_layer}, batch_size={batch_size} ...")
		with torch.no_grad():
			for i in range(0, len(name_seq_pairs), batch_size):
				print(i)
				sub = name_seq_pairs[i:i + batch_size]
				_, _, tokens = batch_converter(sub)  # [B, T]
				tokens = tokens.to(device)

				out = esm_model(tokens, repr_layers=[use_layer], return_contacts=False)
				token_reprs = out["representations"][use_layer]  # [B, T, C]

				pad_idx = alphabet.padding_idx
				padding_mask = tokens.eq(pad_idx)  # [B, T]
				token_reprs = token_reprs.masked_fill(padding_mask.unsqueeze(-1), 0.0)

				lengths = (~padding_mask).sum(dim=1).unsqueeze(-1).clamp(min=1)
				seq_repr = token_reprs.sum(dim=1) / lengths  # [B, C]

				all_repr.append(seq_repr.cpu())

		prot_encode = torch.cat(all_repr, dim=0)  # [N_protein, C]

		if cache_path is not None:
			os.makedirs(os.path.dirname(cache_path), exist_ok=True)
			torch.save(prot_encode, cache_path)
			print(f"[ESM2] embeddings saved to {cache_path}")

		return prot_encode

	def _get_protein_encoding(self, args):
		print("[Data] Using ESM-2 embedding")
		esm_model_name = getattr(args, "esm_model_name", "esm2_t33_650M_UR50D")
		esm_model, alphabet, batch_converter = init_esm2_tokenizer(esm_model_name)
		...
		prot_token_ids = []
		name_seq_pairs = [(f"prot_{idx}", seq) for idx, seq in enumerate(self.seqs)]
		_, _, batch_tokens = batch_converter(name_seq_pairs)
		
		for i in range(batch_tokens.size(0)):
			tok = batch_tokens[i]
			not_pad = (tok != pad_idx).nonzero(as_tuple=False).view(-1)
			if not_pad.numel() == 0:
				tok_trim = tok[:1]
			else:
				max_pos = not_pad.max().item()
				tok_trim = tok[:max_pos + 1]
			prot_token_ids.append(tok_trim.clone())
		
		if self.mapPath is None:
			symbol = re.split('/|[.]', self.seqPath)[-2]
			cache_prefix = f"features/{symbol}"
		else:
			cache_prefix = self.mapPath
		
		cache_path = cache_prefix + "_esm_embed.pt"
		device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
		
		prot_encode = self.encode_with_esm2(
			esm_model=esm_model,
			alphabet=alphabet,
		...
		print(f"ESM-2 initialized: vocab_size={vocab_size}, embed_dim={prot_encode.size(1)}")
		
		return prot_encode, prot_token_ids, vocab_size, pad_value, alphabet
	...
	def embed_prot(self,prot_node, prot_edge, prot_kneg):
		...
		vae_model = CodeBook(param, DataLoader(prot_strut_data, batch_size=512, shuffle=False,collate_fn=collate)).to(device)#collate_fn=collate
		vae_model.load_state_dict(torch.load('data/vae_model.ckpt'))
...
# protein_data, ppi_g, ppi_list, labels, ppi_split_dict = load_data(param['dataset'], param['split_mode'], param['seed'])
	...
	def save_valid_set(self,args):
		...
		if args.split_type != 'read' or args.i3[-4:] == 'data':
			valid_pair = []
			train_pair = []
	...
	def construct_training_set(self,test_indices):
		...
		intersection = set(training_indices).intersection(set(test_indices))
		assert len(intersection) == 0, f"error, found {len(intersection)} overlapping indices between train and test sets"
		
		return training_indices

	def _validate_split(self):
		train_set = set(self.split_dict['train_index'])
		valid_set = set(self.split_dict['valid_index'])
		
		overlap = train_set.intersection(valid_set)
		assert len(overlap) == 0, f"Found {len(overlap)} overlapping samples between train and validation sets"
		
		total = len(train_set) + len(valid_set)
		assert total == len(self.edgeList), f"Split incomplete: {total} != {len(self.edgeList)}"
		
		print(f" Split validation passed: {len(train_set)} train, {len(valid_set)} valid, no overlap")
	...
	with open(relPath,'r') as file:
		...
		for line in lines:
			...
			if name2index[protein1] == -1 or name2index[protein2] == -1:
				...
				exit()
			newPair = sorted_pair(protein1,protein2)
...
def seqEncoding(seqs:list,maxLen=512,modelPath='src/vec5_CTC.txt',w2vPath = 'src/wv_swissProt_size_20_window_16.model',PSSM=None):
	...
	fMatrix = CalAAC(seqs)
	#fMatrix = np.concatenate((fMatrix,CalPAAC(seqs)),axis=1)  
	fMatrix = np.concatenate((fMatrix,CalCTDT(seqs)),axis=1)
